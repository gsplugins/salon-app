...
class BarberShopProfileController extends Controller
{
    ...
    public function stats(Request $request): JsonResponse
    {
        ...
        $bookingsToday = SalonBooking::query()
            ->where('shop_id', $id)
            ->whereBetween('starts_at', [$todayStart, $todayEnd])
            ->count();

        $bookingsWeek = SalonBooking::query()
            ->where('shop_id', $id)
            ->whereBetween('starts_at', [$weekStart, $weekEnd])
            ->count();

        $completedWeek = SalonBooking::query()
            ->where('shop_id', $id)
            ->where('status', BookingStatus::Completed)
            ->whereBetween('starts_at', [$weekStart, $weekEnd])
            ->count();

        $pendingCount = SalonBooking::query()
            ->where('shop_id', $id)
            ->where('status', BookingStatus::Pending)
            ->where('starts_at', '>=', now())
            ->count();

        $revenueWeek = (int) SalonBooking::query()
