...
const SHOP_ROW_SELECT =
  "id,owner_user_id,name,slug,description,is_active,settings,phone,email,address,latitude,longitude,photos,parent_shop_id";
...
async function fetchShopBySlug(slug: string): Promise<ShopRow | null> {
  const res = await supabaseAdmin.from("shops").select(SHOP_ROW_SELECT).eq("slug", slug).maybeSingle();
...
async function primaryShopForUser(userId: string, role: string): Promise<ShopRow | null> {
  ...
  if (mid != null) {
    const s = await supabaseAdmin.from("shops").select(SHOP_ROW_SELECT).eq("id", mid).maybeSingle();
  ...
  if (staffRow.data?.shop_id != null) {
    const s = await supabaseAdmin.from("shops").select(SHOP_ROW_SELECT).eq("id", staffRow.data.shop_id).maybeSingle();
    if (s.data) return s.data as ShopRow;
  }

  if (role === "super_admin") {
    const any = await supabaseAdmin.from("shops").select(SHOP_ROW_SELECT).order("id").limit(1).maybeSingle();
